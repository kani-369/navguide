...
export function Dashboard() {
  ...
  const [budgetLimit, setBudgetLimit] = useState(user?.preferences?.budget || 100000)
  ...
  const filteredTasks = useMemo(() => {
    ...
    // Sort: High → Medium → Low, then Pending before Completed
  ...
  const getInterestName = (id) => {
    const list = { coding: 'Coding', ai: 'AI & ML', business: 'Business', design: 'Design', marketing: 'Marketing', finance: 'Finance', humanities: 'Humanities', engineering: 'Engineering' }
    return list[id] || id
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-cream">
      ...
      {/* ── Next Best Action ── */}
      {nextAction && (
        <div className="relative p-5 rounded-2xl border border-sand/20 bg-gradient-to-r from-sand/5 via-black/20 to-transparent overflow-hidden">
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              <div className="flex items-center space-x-2 mb-1">
                <span className="text-[10px] text-sand font-black uppercase tracking-widest">⚡ Next Best Action</span>
              ...
              <p className="text-base font-bold text-cream">{nextAction.task.title}</p>
              {nextAction.linkedGoal && (
                <p className="text-xs text-cream/50 mt-0.5 flex items-center space-x-1">
                  <Target size={11} />
                  <span>Goal: {nextAction.linkedGoal}</span>
                </p>
              )}
              <p className="text-xs text-cream/40 mt-2 leading-relaxed">{nextAction.advice}</p>
            </div>
            <div className="text-right shrink-0">
              <p className="text-[10px] text-cream/40">Progress</p>
              <p className="text-lg font-black text-mint">{progressPct}%</p>
              <p className="text-[9px] text-cream/30">{completedCount}/{totalCount} tasks</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="xl:col-span-7 space-y-5">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" onClick={() => setShowAddTask(!showAddTask)}
              className="py-1.5 px-3 text-xs space-x-1.5">
              <Plus size={13} />
              <span>Add Task</span>
            </Button>
          </div>
          ...
          {showAddTask && (
            <div className="p-4 glass-card rounded-xl border-mint/20 space-y-3">
              <input
                ...
                className="w-full bg-black/30 border border-cream/10 rounded-lg px-3 py-2 text-sm text-cream placeholder-cream/30 outline-none focus:border-mint transition-colors"
              />
              <div className="flex flex-wrap gap-2">
                ...
                <select
                  ...
                  className="text-xs px-3 py-1 rounded-lg border border-cream/10 bg-black/30 text-cream/50 outline-none flex-1 min-w-[130px]"
                >
              ...
              <div className="flex space-x-2">
                <Button variant="primary" onClick={handleAddTask} className="py-1.5 px-4 text-xs flex-1">Add Task</Button>
                <Button variant="ghost" onClick={() => setShowAddTask(false)} className="py-1.5 px-3 text-xs">Cancel</Button>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            {filteredTasks.length === 0 ? (
              <div className="text-center py-10 text-cream/30 text-sm">
              ...
              return (
                <div key={task.id} className={`group flex items-start space-x-3 p-3.5 rounded-xl border transition-all duration-200 ${
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold ${task.status === 'Completed' ? 'line-through text-cream/40' : 'text-cream'}`}>
                      {task.title}
                    </p>
                    <div className="flex items-center space-x-2 mt-1">
                      ...
                      {linkedGoal && (
                        <span className="text-[9px] text-cream/30 flex items-center space-x-1">
                          <Target size={9} />
                  ...
                  <button onClick={() => deleteTask(task.id)}
                    className="opacity-0 group-hover:opacity-100 text-cream/20 hover:text-coral transition-all shrink-0">
                    <Trash2 size={14} />
                  </button>
                </div>
              )
                ...
                return (
                  <div key={goal.id} className="p-3 bg-white/5 rounded-xl border border-white/5">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-xs font-semibold text-cream">{goal.title}</span>
                      <span className="text-[10px] text-mint font-bold">{pct}%</span>
                    </div>
                    <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-gradient-to-r from-mint to-sand h-full rounded-full transition-all duration-500"
                        style={{ width: `${pct}%` }} />
                    </div>
                    <span className="text-[9px] text-cream/30 mt-1 block">{goal.type} · {done}/{linked.length} tasks done</span>
                  </div>
                )
              })}
            </CardContent>
          ...
          <Card className="glass-card max-w-full">
            <CardHeader className="mb-3">
              <div className="flex items-center justify-between">
                ...
                <a href="/colleges" className="text-[10px] text-mint hover:text-sand transition-colors font-semibold">View All →</a>
              </div>
              <div className="flex items-center space-x-2 mt-2">
                <span className="text-[10px] text-cream/40">Budget/yr:</span>
                <input type="range" min="10000" max="300000" step="5000" value={budgetLimit}
                  onChange={e => setBudgetLimit(parseInt(e.target.value))}
                  className="flex-1 cursor-pointer" />
                <span className="text-[10px] text-sand font-bold w-10 text-right">₹{(budgetLimit/1000).toFixed(0)}k</span>
              </div>
            </CardHeader>
            <CardContent className="space-y-2.5">
              {topColleges.length === 0 ? (
                <p className="text-xs text-cream/40 text-center py-4">No matches in current budget. Try increasing the slider.</p>
              ) : topColleges.map((c, idx) => (
                <div key={c.id} className="flex items-center space-x-3 p-3 bg-white/5 rounded-xl border border-white/5 hover:border-mint/20 transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-cream truncate">{c.college_name}</p>
                    <p className="text-[10px] text-cream/40 truncate">{c.top_course} · ₹{(c.annual_fee/1000).toFixed(0)}k/yr</p>
                  </div>
                  <div className="flex items-center space-x-1 shrink-0">
                    <Star size={11} className="text-sand fill-sand" />
                    <span className="text-[10px] text-sand font-bold">{c.rating}</span>
                  </div>
                </div>
              ))}
            </CardContent>
